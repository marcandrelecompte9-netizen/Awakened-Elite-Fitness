...
// ── PERSISTENCE ─────────────────────────────────────────────────────────
// ⚠️ Le MODE JEU est l'interrupteur MAÎTRE de toute la couche RPG.
// Ce garde-fou est indispensable car les deux réglages n'ont PAS la même
// portée : 'fitproGameMode' est PAR PROFIL (il figure dans GAME_KEYS) alors que
// cette clé-ci était GLOBALE. Un profil dont le jeu était éteint héritait donc
// du drapeau laissé actif par un autre profil — et recevait des notifications
// de Faille alors que le mode jeu était désactivé chez lui.
function getAdventureEnabled() {
    try {
        if (typeof window.rpgEnabled === 'function') { if (!window.rpgEnabled()) return false; }
        else if (localStorage.getItem('fitproGameMode') !== '1') return false;
    } catch (e) {}
...
// (v551) Doublon de getItemRankValue() retiré ici — définition identique
...
function equipItem(invId) {
    ...
    eq[item.slot] = invId;
    saveEquipped(eq);
    // v680 : équiper modifie les stats affichées (getPlayerEquipStats entre dans
    // awakGetTotalStats). On émet rpg:statsChanged pour que la Carte de Chasseur
    // suive — jusqu'ici seuls les appelants redessinaient la Power Card, la
    // Carte de Chasseur restait figée sur l'ancienne valeur jusqu'au changement
    // d'onglet. Émis au point de mutation → tous les chemins d'équipement couverts.
    if (typeof window.awakEmit === 'function') window.awakEmit('rpg:statsChanged', { source: 'equip', invId: invId });
    return { success: true };
}
function unequipSlot(slot) {
    const eq = getEquipped(); eq[slot] = null; saveEquipped(eq);
    // v680 : voir equipItem — le retrait modifie aussi les stats affichées, la
    // Carte de Chasseur doit suivre.
    if (typeof window.awakEmit === 'function') window.awakEmit('rpg:statsChanged', { source: 'unequip', slot: slot });
}
...
// 👨‍👩‍👧 ONGLET FAMILLE — coopération familiale & duo, INDÉPENDANT du mode jeu.
// Monte les mêmes cartes que l'onglet Aventure (Boss Familial + Défis de Duo)
// mais sans aucune condition sur getAdventureEnabled() : accessible à tous,
// même si le RPG/Aventure est désactivé.
function renderFamilyTab() {
    const container = document.getElementById('familyContainer');
    if (!container) return;

    const profileCount = (window.AwakFamily && typeof window.AwakFamily.profileCount === 'function')
        ? window.AwakFamily.profileCount()
        : ((window.AwakFamilyBoss && typeof window.AwakFamilyBoss.profileCount === 'function')
            ? window.AwakFamilyBoss.profileCount() : 1);

    // Onglet Famille = réseau familial (relations typées). Le Boss Familial
    ...
    const nudgeCard = (window.AwakFamily && typeof window.AwakFamily.renderNudgeCard === 'function')
        ? window.AwakFamily.renderNudgeCard() : '';
    const challengeCard = (window.AwakFamilyChallenge && typeof window.AwakFamilyChallenge.renderCard === 'function')
...
// Rafraîchit la vue coopérative où qu'elle soit affichée (onglet Jeu ET/OU
// onglet Famille). Les actions des cartes (attaque, réclamation, liaison…)
// appellent cette fonction pour que les deux onglets restent synchronisés.
function refreshCoopViews() {
    try { if (typeof renderFamilyTab === 'function') renderFamilyTab(); } catch (e) {}
    try {
        const adv = document.getElementById('adventureContainer');
        if (adv && typeof renderAdventureTab === 'function' && typeof getAdventureEnabled === 'function' && getAdventureEnabled()) {
            renderAdventureTab();
        }
    } catch (e) {}
}
window.renderFamilyTab = renderFamilyTab;
window.refreshCoopViews = refreshCoopViews;
...
function renderAdventureTab() {
    ...
    container.innerHTML = familyBossCard
        + monstersCard
        + riftsCard
        + companionsCard
        + consumablesCard
