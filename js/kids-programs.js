...
(function () {
  ...
  function renderTab() {
    var cards = KIDS_PROGRAMS.map(function (p) {
      var sessionList = p.sessions.map(function (s, i) {
        return '<div style="margin-top:8px;padding:10px 12px;background:rgba(255,255,255,0.03);border-radius:10px;">'
          + '<div style="font-size:0.8em;font-weight:800;color:#fff;margin-bottom:5px;">Jour ' + (i + 1) + ' · ' + esc(s.name) + '</div>'
      ...
      return '<div style="background:linear-gradient(160deg,' + p.color + '18,#0d0d12);border:1px solid ' + p.color + '55;border-radius:18px;padding:18px;margin-bottom:14px;">'
        ...
        +   '<div style="font-size:0.72em;color:' + p.color + ';font-weight:700;">' + esc(p.focus) + ' · ' + p.days + ' jours/sem</div></div>'
        + '</div>'
    ...
    return '<div style="margin-bottom:14px;">'
      + '<h2 style="font-size:1.3em;font-weight:900;color:#fff;margin:0 0 4px;">🌟 Mes programmes</h2>'
      + '<p style="font-size:0.8em;color:#94a3b8;margin:0;line-height:1.4;">Des entraînements amusants, tout en s\'amusant et en bougeant. Demande à un adulte de t\'accompagner !</p>'
      + '</div>'
      + cards;
  }
